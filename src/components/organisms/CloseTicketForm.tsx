...
import CostSummary from "@/components/molecules/CostSummary";
import PartsEditor, { type Part } from "@/components/organisms/PartsEditor";
...
import TextAreaLabelField from "@/components/molecules/TextAreaLabelField";
...
type Props = {
  ...
  data?: TicketLite;
  onClose: () => void;
  onSubmit?: (payload: CloseTicketPayload) => Promise<void> | void;
};
...
export default function CloseTicketForm({
  ...
  const [form, setForm] = React.useState<CloseTicketPayload>({
    ringkasanSolusi: "",
    akarMasalah: "",
    pengujian: "",
    parts: [],
  ...
  const set = <K extends keyof CloseTicketPayload>(
    k: K,
    v: CloseTicketPayload[K]
  ) => setForm((s) => ({ ...s, [k]: v }));
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {data?.id && (
            <span className="text-xs px-2 py-1 border rounded-none border-[var(--mono-border)]">
              ID: {String(data.id)}
            </span>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <SelectLabelField
          ...
          onChange={(e) =>
            set(
              "finalStatus",
              e.target.value as CloseTicketPayload["finalStatus"]
            )
          }
          options={[

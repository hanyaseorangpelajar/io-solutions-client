...
export default function AuditRepositoryPage() {
  // Filter sederhana: deviceType (dari tag) & tag spesifik
  ...
  const filtered = useMemo(() => {
    return cards.filter((c) => {
      const byDevice = device === "all" ? true : c.deviceType === device;
      const byTag = tag === "all" ? true : c.tags.includes(tag);
      return byDevice && byTag;
    });
  }, [cards, device, tag]);
